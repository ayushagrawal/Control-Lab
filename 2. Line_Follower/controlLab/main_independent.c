...
const unsigned char nominalValueLeft = 0;
const unsigned char nominalValueRight = 0;
...
void motor_control(void)
{
	/*
	if(pidLeft > 0) {
		velR = MAX_SPEED * max(min((1-pid/norm), MAX_SPEED/2), 0) / 4 ;
		velL = MAX_SPEED / 2;
	}
	else if (pid < 0) {
		velR = MAX_SPEED / 2;
		velL = MAX_SPEED * max(min((1-pid/norm), MAX_SPEED/2), 0) /4 ;
	}
	else {
		velR = velocity_right;
		velL = velocity_left;
	}*/
	velL = max(min(velocity_left + pidLeft, MAX_SPEED), 0);
	velR = max(min(velocity_right + pidRight, MAX_SPEED), 0);
